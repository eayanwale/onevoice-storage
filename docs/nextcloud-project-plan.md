...
- ✅ **nginx rate limiting** — `limit_req_zone` (10r/s, zone `nextcloud_limit`) in `/etc/nginx/conf.d/rate-limit.conf`, `limit_req zone=nextcloud_limit burst=20 nodelay;` applied to `location /` in `nextcloud.conf`. Originally applied by hand over SSH; now also written into `packer/setup.sh` (see changelog.md's "Bake config drift into the golden AMI" entry) — code complete, not yet pushed/applied.
...
- **nginx rate limiting → AMI:** ~~applied manually over SSH, not yet folded into `packer/setup.sh`~~ — **code written** (`packer/setup.sh`, plus cloudflared and the MCP server/maintenance timer alongside it — see changelog.md), sitting on a feature branch pending push/PR; blocked on a GitHub App permission issue, see the new entry below
- **Cloudflare Tunnel / MCP server / maintenance timer → AMI:** same config-drift problem as the rate limiting above — all three were only ever applied by hand over SSH and documented in `docs/nextcloud-maintenance-automation.md`, never version-controlled. Code now written in `packer/setup.sh`, `packer/nextcloud.pkr.hcl`, and `terraform/scripts/user-data.sh` (5 new SSM-sourced secrets pulled at boot). Not yet pushed, PR'd, or applied — see changelog.md for full status and blockers
- **GitHub App write access:** issue/PR writes and `git push` both return `403 Resource not accessible by integration` for this session's GitHub integration; account shows no GitHub App installed. Blocks pushing the config-drift branch above and blocks filing/updating issues via API until resolved