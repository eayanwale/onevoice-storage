...
resource "aws_subnet" "priv-a" {
  vpc_id            = aws_vpc.onevoice-vpc.id
  cidr_block        = var.subnet_cidrs.c
  availability_zone = "${var.aws_region}a"

  tags = {
    Name = "${var.organization}-${var.environment}-priv-a"
  }
}
...
resource "aws_subnet" "priv-b" {
  ...
  tags = {
    Name = "${var.organization}-${var.environment}-priv-b"
  }
}
...
resource "aws_route_table" "priv-rt" {
  ...
  tags = {
    Name = "${var.organization}-${var.environment}-priv-rt"
  }
}
...
resource "aws_security_group" "db-sg" {
  name        = "${var.organization}-${var.environment}-nextcloud-db-sg"
  description = "Allow MySQL from the Nextcloud app server only"
  vpc_id      = aws_vpc.onevoice-vpc.id
